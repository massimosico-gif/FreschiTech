...
#set page(
  ...
  footer: context {
    ...
    v(-1mm)
    align(right)[Pagina #counter(page).at(here()).first() di #counter(page).final().first()]
    
    line(length: 100%, stroke: 0.5pt + accent-color)
    
    // Spostiamo il footer fuori dai margini standard (portandoli a 7mm come in Python)
    pad(left: -13mm, right: -14mm)[
      // Info aziendali
      #align(center)[
        #set text(font: "Plus Jakarta Sans", fill: logo-blue, size: 9.2pt, tracking: 0.42pt)
...
#grid(
  ...
  [ // Destra: Blocco Destinatario
    ...
    #grid(
      ...
      grid.cell(colspan: 3)[#client.street],
...
#v(1mm)
*#invoice.billing_street, #invoice.billing_zip_code - #invoice.billing_city (#invoice.billing_province)*
#v(5mm)
...
#grid(
  ...
  [
    ...
    #grid(
      ...
      row-gutter: 7.15pt,
      [Codice CIG], [:], [#if invoice.cig != "" and invoice.cig != none [#invoice.cig] else [-]],
      [Codice CUP], [:], [#if invoice.cup != "" and invoice.cup != none [#invoice.cup] else [-]],
    )
  ],
  [
    #set align(right)
    ...
    #align(right)[
      #grid(
        ...
        row-gutter: 7.15pt,
        align: (left, right),
        [Totale imponibile:], [#h(2mm) €.#h(2mm) #format-money(totals.netto).replace("€ ", "") =],
        ..(if totals.withholding != "0,00" {
          ([Ritenuta d'acconto:], [#h(2mm) - €.#h(2mm) #format-money(totals.withholding).replace("€ ", "") =])
        } else { () }),
        [*Totale Fattura:*], [*#h(2mm) €.#h(2mm) #format-money(totals.final).replace("€ ", "") =*]
      )
    ]
  ]
)
