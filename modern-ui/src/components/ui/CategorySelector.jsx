...
const CategorySelector = ({ categories, value, onChange, onAddNew }) => {
    ...
    <div className="relative" ref={containerRef}>
      ...
      >
        ...
        <span className={selectedCategory ? 'text-slate-700' : 'text-slate-400'}>
          {selectedCategory ? selectedCategory.label : 'Seleziona una categoria...'}
        </span>
          ...
          <div className="max-h-60 overflow-y-auto no-scrollbar py-2">
            {filteredCategories.length > 0 ? (
              filteredCategories.map(category => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all group ${
                    value === category.id ? 'bg-accent/5 text-accent' : 'hover:bg-slate-50 text-slate-600'
                  }`}
                >
              ...
              <div className="p-8 text-center text-slate-400 flex flex-col items-center gap-3">
                <p className="text-[0.75rem] font-black uppercase tracking-widest text-slate-400">Nessuna categoria trovata</p>
                {onAddNew && (
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  >
                    {search ? `+ Aggiungi "${search}"` : '+ Aggiungi Nuova Categoria'}
                  </button>
                )}
              </div>
            )}
